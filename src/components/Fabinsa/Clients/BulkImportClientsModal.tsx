...
export function BulkImportClientsModal({ onClose, onSuccess }: BulkImportClientsModalProps) {
  ...
  const parseExcel = async (file: File): Promise<ClientImportRow[]> => {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const row of rows) {
        try {
          // Verificar si ya existe un cliente con el mismo nombre o CUIT
          let existing = null;
      ...
      setResultMessage(
        `Importación completada: ${imported} clientes importados${errors > 0 ? `, ${errors} errores` : ''}`
      );
  ...
  const downloadTemplate = () => {
    const templateData = [
      {
        'Nombre': 'EVELYN LIARDET',
        'Razon_Social': 'EVELYN LIARDET ALEJANDRO ARIEL',
        'CUIT': '20224009055',
        'Telefono': '',
        'Email': '',
        'Provincia': 'SANTA FE',
      ...
      {
        'Nombre': 'EDUARDO CUENDE',
        'Razon_Social': 'CUENDE EDUARDO DANIEL',
        'CUIT': '20296313905',
        'Telefono': '0291 452-2960',
        'Email': '',
      ...
      {
        'Nombre': 'DOM DISTRIBUCIONES',
        'Razon_Social': 'D.O.M. DISTRIBUCIONES S.A.',
        'CUIT': '33710973739',
        'Telefono': '3413177830',
    ...
    writeFile(wb, 'plantilla_importacion_clientes.xlsx');
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Importar Clientes Masivamente</h3>
          <button
            onClick={onClose}
